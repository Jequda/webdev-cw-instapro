...
export function renderPostsPageComponent({ appEl, posts, user, token, singleUserView }) {
  const locales = { ru }
  ...
  const appHtml = `
    <div class="page-container">
      ...
      ${singleUserView && posts[0] ? `<div class="posts-user-header">
    ...
    likeButton.addEventListener("click", () => {
      ...
      if (!(getUserFromLocalStorage())) {
        ...
        if (posts[index].isLiked === true) {
          likePost({ token, name: user.name, id: posts[index].id })
            .then((updatedPost) => {
          ...
          dislikePost({ token, name: user.name, id: posts[index].id })
            .then((updatedPost) => {
              posts[index].likes = updatedPost.likes
              renderApp()
            })
